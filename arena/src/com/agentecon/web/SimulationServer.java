	...
	public SimulationServer(int port) throws IOException, InterruptedException {
		...
		if (SimulationConfig.isServerConfig()) {
			try {
				...
				this.simulations.add(new GitSimulationHandle("meisser", "course", "ex9-basic-2", true));
				this.simulations.add(new GitSimulationHandle("meisser", "course", "ex9-competitive", true));
			} catch (IOException e) {
